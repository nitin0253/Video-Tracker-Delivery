...
export default async function handler(req, res) {
  // Only allow this to be triggered by Vercel Cron (or manually with the secret)
  ...
  try {
    ...
    res.status(200).json({ ok: true, total: rows.length, lastSynced: new Date(now).toISOString() });
  } catch (err) {
